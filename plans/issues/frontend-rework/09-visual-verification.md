...
**Outcome, 2026-07-26:** the owner configured the MCP server and reloaded plugins during the run,
and it became available. This slice was then run against the built image. See
[`09-verification-record.md`](09-verification-record.md) for what was confirmed, what was fixed —
including virtualisation not binding at all behind a `min-h-screen` root — and the two findings
